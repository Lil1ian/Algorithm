class Solution {
    ...
    public boolean exist(char[][] board, String word, int currentIndex, int row, int col) {
        if(currentIndex == word.length()) {
            return true;
        }
    
        if(row < 0 || row >= board.length || col < 0 || col >= board[0].length || board[row][col] != word.charAt(currentIndex)) {
            return false;
        }
